...
import { getAnuarioEstadistico, exportAnuarioCsv } from '../services/anuario-service.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function renderCards(list) {
    ...
    const html = list.map((est) => `
      <article class="station-card-item" id="card-${est.codigo}" data-codigo="${est.codigo}">
        ...
        <div class="station-card-actions">
          <button class="btn-card-ver-datos" data-codigo="${est.codigo}">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
    ...
    cardsContainer.querySelectorAll('.station-card-item').forEach((card) => {
      ...
      const btnVer = card.querySelector('.btn-card-ver-datos');
      if (btnVer) {
        btnVer.addEventListener('click', () => openModal(estacion));
      }
